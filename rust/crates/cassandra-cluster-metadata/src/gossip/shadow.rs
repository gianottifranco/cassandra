...
use crate::gossip::messages::{GossipDigestSyn, JavaGossipCodec};
...
pub async fn execute_shadow_round(
    ...
    let syn_payload = JavaGossipCodec::encode_syn(&syn)
    ...
    for seed in &seeds {
            ...
            .await
        {
            Ok(ack_msg) => {
                ...
                match JavaGossipCodec::decode_ack(&ack_msg.payload) {
                    Ok(ack) => {
                        seeds_responded += 1;
