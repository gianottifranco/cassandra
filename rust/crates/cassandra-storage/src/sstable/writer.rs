...
//!   [PARTITION_DELETION_MARKER: 1B][marked_for_delete_at: i64][ldt: i32]?
//!   For each row:
...
use super::tombstone_serializer::{write_partition_deletion, write_range_tombstone_marker};
use crate::memtable::partition::{Cell, PartitionData, Row};
use crate::partitions::filtered_partition::FilteredPartition;
use crate::rows::unfiltered::{ColumnData, RowData, Unfiltered};
...
impl SSTableWriter {
    ...
    /// Write materialized unfiltered partitions to disk, preserving range tombstone markers.
            ...
            for item in &partition.items {
                match item {
                    ...
                    Unfiltered::Marker(marker) => {
                        data_offset +=
                            write_range_tombstone_marker(&mut data_file, marker, &mut data_crc)?;
                    }
                }
            }
...
pub(crate) fn row_data_to_row(row_data: &RowData) -> Row {
    let mut cells = Vec::new();
    for column in row_data.columns.values() {
        match column {
            ColumnData::Simple(cell) => cells.push(Cell::from(cell)),
            ColumnData::Complex(complex) => {
                cells.extend(complex.cells.values().map(Cell::from));
            }
        }
    }

    Row {
        clustering_key: row_data.clustering_key.clone(),
        cells,
...
mod tests {
    ...
    #[test]
    fn write_includes_partition_deletion_marker() {
        let dir = TempDir::new().unwrap();
        let desc = SSTableDescriptor::new(dir.path(), "ks", "t1", 1);
        let writer = SSTableWriter::new(desc.clone());
        ...
        writer.write(&[(b"pk".to_vec(), pd)]).unwrap();

        let data = fs::read(desc.component_path(Component::Data)).unwrap();
        assert!(data.contains(&super::super::tombstone_serializer::PARTITION_DELETION_MARKER));
    }
    ...
    fn write_filtered_partitions_preserves_range_tombstone_markers() {
        ...
        let reader = super::super::reader::SSTableReader::open(desc).unwrap();
